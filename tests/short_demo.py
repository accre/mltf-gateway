...
def refresh_token():
    ...
    token_info = None
    for _ in range(int(auth_url_response['expires_in'] / auth_url_response['interval'])):
        r = requests.post(URL + "/auth/poll", data={'device_code': auth_url_response['device_code']})
        r.raise_for_status()
        ret = r.json()
        print(ret)
...
auth_get = functools.partial(requests.get, headers={'Authorization': f"Bearer {token['access_token']}"})
auth_post = functools.partial(requests.post, headers={'Authorization': f"Bearer {token['access_token']}"})
...
r = auth_post(URL + "/api/projects", data={'name': "testreq1"}, files={'file': open(project_tarball, 'rb')})
print(r)
print(r.json())
r.raise_for_status()
