...
def oauth2_callback():
    ...
    if (
        not (is_token_flow or is_api_flow)
    ...
    if (
        not (is_token_flow or is_api_flow)
        and request.args['state'] != session.get('oauth2_state')
    ):
        abort(401)
    ...
    if is_token_flow:
        ...
        redirect_uri = url_for('auth.oauth2_callback', token=True, api=True, _external=True)
    else:
        redirect_uri = url_for('auth.oauth2_callback', _external=True)
    ...
    handle_user_login(email, oauth_response, request.args['code'], is_token_flow=is_token_flow)
    if is_api_flow:
