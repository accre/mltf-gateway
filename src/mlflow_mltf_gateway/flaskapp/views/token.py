...
def get_current_tokens():
    ...
    if not current_user.is_authenticated:
        return []
    return db.session.scalars(db.select(Token).where(Token.user_id == current_user.id)).all()
...
def index():
    ...
    return render_template('token.html', tokens=get_current_tokens(), new_token_url=new_token_url)
