...
def is_jwt_expired(token):
    """
    Helper function since we need to check so many times. Returns true if token has
    expired or will expire in
    """
    return get_jwt_expiration(token) - int(time.time()) <= 60
...
class ReloadableToken:
    ...
    def reload_token(self):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to reload JWT from {self.token_path}") from e
...
class ClientRefreshToken:
    ...
    def reload_token(self) -> bool:
        ...
        self.expiration = get_jwt_expiration(token)
        self.token = token
        return True
...
class SSAMExecutor(ExecutorBase):
    ...
    def _ssam_request(
        ...
        try:
            ...
            print(f"POSTING TO SLURM: data: {multipart_form_data}\nheaders: {headers}")
            response = requests.post(
