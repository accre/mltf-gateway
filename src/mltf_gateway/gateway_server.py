...
from mltf_gateway.data_classes import (
    MovableFileReference,
    RunReference,
    GatewayRunDescription,
)
from mltf_gateway.executors.base import get_script, ExecutorBase
from mltf_gateway.executors.local_executor import LocalExecutor
from mltf_gateway.executors.slurm_executor import SLURMExecutor
from mltf_gateway.executors.ssam_executor import SSAMExecutor
from mltf_gateway.submitted_runs.server_run import (
    ServerSideSubmittedRunDescription,
)
from mltf_gateway.utils import get_tracking_uri
...
class GatewayServer:
    ...
    def get_health(self):
        if hasattr(self.executor, "get_health"):
            return self.executor.get_health()
        else:
            return {}
    ...
    def runid_to_reference(self, run_id: int):
        for run in self.runs:
            if run.gateway_id == run_id:
                return run
        raise IndexError()
    ...
    def wait(self, run_ref):
        ...
        :return: Was the task successful
        """
        return self.reference_to_run(run_ref).submitted_run.wait()
    ...
    def get_execution_snippet(
        ...
        :param tracking_uri: Tracking server the job should speak to
        ...
        tracking_uri = run_desc.tracking_uri
        if not tracking_uri:
            raise RuntimeError("Need to provide a tracking URI")

        env_vars = [f"export MLFLOW_TRACKING_URI={shlex.quote(tracking_uri)}"]
        env_vars.append(f"export MLFLOW_ENABLE_SYSTEM_METRICS_LOGGING=true")

        if run_desc.run_id not in ("", "UNKNOWN"):
            env_vars.append(f"export MLFLOW_RUN_ID={shlex.quote(tracking_uri)}")
        if runtime_token:
            env_vars.append(
                f"export MLFLOW_TRACKING_TOKEN={shlex.quote(runtime_token)}"
            )

        env_vars.append(f"export MLFLOW_TRACKING_URI={shlex.quote(tracking_uri)}")
