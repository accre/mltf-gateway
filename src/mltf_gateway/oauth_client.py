...
def poll_token(device_code, interval=0):
    """Poll for access token using device code"""

    # Wait for 5 seconds to poll tokens
    if not interval:
        interval = os.environ.get('MLTF_TEST_TOKEN_POLL_INTERVAL', 5)
    data = {
        "client_id": CLIENT_ID,
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ret = response.json()
                for x in ('access_token', 'expires_in'):
                    if x not in ret:
                        raise ValueError(f"Auth server didn't include {x} in response")
                return ret
...
def authenticate_with_device_flow() -> Optional[Dict[str, Any]]:
    ...
    if stored and not token_expired(stored):
    ...
    if not device_response:
        raise RuntimeError("Failed to obtain OAuth2 device code")

    device_code = device_response.get("device_code")
    user_code = device_response.get("user_code")
    verification_uri = (
        device_response.get("verification_uri") + f"?user_code={user_code}"
    )
    ...
    if not os.environ.get("MLTF_TEST_DISABLE_WEBBROWSER", None):
        try:
            webbrowser.open(verification_uri)
        except Exception as e:
            # Most of our users will be headless, so don't complain if we cant
            # open a browser
            pass
...
def is_authenticated() -> bool:
    ...
    # If we're only talking to a local interface then skip authentication
    if os.environ.get("MLFLOW_TRACKING_URI", None) == "file://":
        return True
    creds = get_stored_credentials()
    if not creds:
        return False
